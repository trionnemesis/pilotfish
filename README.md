...
The policy layer requires a schema-valid Task Envelope and the canonical deterministic router before dispatch. A dispatch brake keeps root-cause discovery, trace-driven debugging, and tightly coupled state work in the main session when a worker would only reconstruct the same evidence; stable work is still delegated when the net benefit is positive. It treats `REFINE`, `TAKEOVER`, and `BLOCK` as control-plane outcomes rather than invented roles, keeps security work on its fixed lane, requires explicit approval before source edits when the Plan calls for it, lets every named role take its model only from its agent definition, and gates non-trivial work behind fresh-context verification.
...
| Does the orchestrator ever do work itself? | Yes — quick reads, decisions, root-cause discovery, trace-driven debugging, tightly coupled state work, and anything you explicitly asked *it* to judge. Delegation must have a positive net benefit. |
...
| Doesn't spawning agents cost extra? | Yes — every spawn is a fresh context that re-reads its slice of the codebase, and spec-writing costs main-session tokens. The dispatch brake keeps tightly coupled diagnosis inline; delegation remains useful for stable volume work, real parallelism, isolated ownership, and fresh-context independence when those benefits outweigh coordination. |
