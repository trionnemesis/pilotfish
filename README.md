...
The policy layer first applies a dispatch brake: unstable contracts, overlapping ownership, root-cause exploration, runtime tracing, and tightly coupled state work stay with the orchestrator. A role match makes work eligible, not mandatory. Remaining work is chosen by net benefit, so a cheap bounded worker can still be worthwhile when it saves frontier-model usage even if direct execution is slightly faster. Small repository scans remain inline by default; stable multi-file repetition, genuinely large independent scans, and fresh verification retain explicit delegation paths. Stable delegations are specified completely in one shot (including the *why*), take every named role's model only from its agent definition, schedule independent work in the background, and reserve foreground agents for immediate dependencies whose net benefit remains positive.
...
| I also installed a delegation-planning skill | Treat that as a second orchestration authority. Later Skill output can conflict with pilotfish's global policy; the public experiment observed this with GPT-5.6 Sol and `baton-dispatch` v0.1.1. Prefer one authority, or add an explicit project compatibility rule. pilotfish does not disable user skills. |
...
| Doesn't spawning agents cost extra? | Yes — every spawn is a fresh context that re-reads its slice of the codebase, and spec-writing costs main-session tokens. The dispatch brake rejects work whose diagnosis and implementation share the same evidence, and keeps roughly a dozen short files inline unless latency or independent-perspective requirements justify fan-out. It still delegates stable bulk edits when cheaper execution outweighs coordination; the public positive control reduced the reported cost field by 36.01% with a 7.92% wall-time trade-off. |
...
| [benchmarks/dispatch-brake/positive-controls/README.md](./benchmarks/dispatch-brake/positive-controls/README.md) | English + data | Detailed proof that the brake preserves useful mechanical delegation while rejecting small-scan fan-out |
