...
The existing `verifier` role gains a read-only Plan-readiness mode (`READY` / `REVISE`) alongside outcome verification (`CONFIRMED` / `REFUTED`). It never writes the Plan or fixes implementation, so Plan synthesis and final judgment stay in the main session. Named-role model ownership, leaf-agent restrictions, background scheduling, worktree isolation, and the install approval boundary are unchanged.

Fix the long-running-process handoff based on four direct harness trials reported by [@dromsak](https://github.com/dromsak) in PR #10. Executor roles no longer use `nohup`, `setsid`, trailing `&`, or subagent-side background commands: detaching escapes task tracking and orphans the result. A subagent that cannot finish a foreground command within 10 minutes reports the exact command back; the main orchestrator owns tracked background execution. Agents likely to run a long command must themselves be spawned in the background, because a timeout-promoted command in a foreground-spawned agent is terminated shortly after that agent returns.

This release publishes the complete bilingual experiment behind the dispatch decision: negative and positive-control fixtures, neutral prompts, rejected policy iterations, exact Agent tool inputs, normalized traces, model usage, timing, client-reported cost fields, raw-stream hashes, commands, correctness results, and limitations. A hard direct-speed veto suppressed useful delegation; the balanced policy still routed a stable 12-file mechanical edit to the cheaper worker, reducing the reported cost field by 36.01% with a 7.92% wall-time trade-off in one run.
...
New policy contract tests lock the phase boundary, approval gate, planning-skill composition, Plan and outcome verifier modes, invocation-specific verdict vocabulary, named-role model ownership, background scheduling, worktree isolation, long-process ownership, and fresh-context verification. The fresh-session two-turn Gate completed zero-write Discovery and main-session Plan synthesis, an Opus readiness review returning `READY`, explicit approval, direct one-file execution selected by Baton, and a separate Opus outcome verifier returning `CONFIRMED`. Both named-role calls omitted invocation-level `model`; `npm test` passed and `REPORT.md` was the only change. The final run took 494.933 seconds and reported $3.906375 in the client cost field; these are single-run observations, not population estimates or an invoice. A first isolation attempt is also disclosed and rejected because its setting-source filter hid the Baton skill. The later long-process fix is hash-separated in the report and relies on its own four direct harness trials rather than being misrepresented as exercised by this small fixture.
